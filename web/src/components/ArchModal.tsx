...
const SLIDES: Slide[] = [
  {
    ...
    body: {
      ...
      es: "Qué es QLab y cómo se construyó. Es un laboratorio cuántico estático: 20 casos, cada uno una corrida real de un circuito que enfrenta un método cuántico contra una línea base clásica honesta. El ciclo de diseño-construcción va de izquierda→derecha — investigar el algoritmo y sus ecuaciones en una ficha, implementarlo como un Problem más adaptadores Solver, precomputar (hornear) cada solver en una traza JSON con semilla, construir la SPA superponiendo esos datos y desplegar en GitHub Pages — y luego iterar. Las seis páginas son un banco de trabajo (App) más cinco páginas-doc que ilustran el método con los mismos diagramas temáticos.",
    },
  ...
  },
  {
    ...
    body: {
      ...
      es: "Dónde corre cada pieza de trabajo. El carril WEB re-simula circuitos unitarios pequeños y limpios en vivo en el navegador con un motor de statevector exacto escrito a mano (Qiskit no puede correr en el navegador — sin wheels de Pyodide). El carril OFFLINE corre los motores reales pesados (Qiskit+Aer, PennyLane, Cirq, Stim) en un .venv local cuando un caso necesita ruido, feed-forward, un loop de optimización o >12 qubits, y commitea una traza con semilla + manifiesto. El carril CÓMPUTO (inactivo, opcional, solo local) puede reproducir un trabajo de QPU real. Los tres emiten UNA forma de artefacto, así que la SPA estática anima cualquiera por un único render — 'vivo' es responder al slider, no otro modelo.",
    },
