        ...
        <div className="method-body">
          ...
          <p>
            {en
              ...
              : "QLab escala la idea a tres qubits, donde existen dos especies no equivalentes. El estado GHZ (|000⟩+|111⟩)/√2 es máximo pero frágil — medir un qubit destruye todo el entrelazamiento — mientras que el estado W (|001⟩+|010⟩+|100⟩)/√3 es robusto y sobrevive la pérdida de un qubit. El resultado de Dür–Vidal–Cirac prueba que no pueden convertirse uno en otro por operaciones locales y comunicación clásica, así que son recursos genuinamente distintos, no dos descripciones de lo mismo. Ambos se ejecutan en vivo: GHZ-3 devuelve solo 000 y 111, W-3 devuelve 001/010/100 a 1/3 cada uno — exactamente la estructura que afirma el build."}
          </p>
          <Callout
        ...
        <div className="method-body">
          ...
          <Eq
            ...
            caption={{
              ...
              es: "La amplitud de todos-ceros de Deutsch–Jozsa decide constante vs balanceada en una consulta; cada ejecución de Simon entrega un y ortogonal al período oculto s.",
            }}
          />
          <p>
            {en
              ...
              : "El algoritmo de Simon es el puente hacia Shor. Su oráculo es 2-a-1 con un período oculto s (f(x) = f(x⊕s)); cada ejecución entrega un y aleatorio con y·s ≡ 0 (mod 2), y unos n−1 y's independientes fijan s de forma única por eliminación gaussiana sobre GF(2). Necesita O(n) consultas cuánticas frente a ~O(2^{n/2}) de búsqueda de colisiones clásica — la primera separación exponencial demostrable, y el ancestro conceptual directo de la búsqueda de orden de Shor. El build ejecuta n=2–3 (4–6 qubits), lee la marginal del registro de entrada y resuelve GF(2) para el único s no nulo; el baseline clásico busca una colisión en su lugar."}
          </p>
          <Callout
            title={en ? "Quantum vs classical —" : "Cuántico vs clásico —"}
            pt={en
              ...
              : "Son separaciones reales de complejidad de consultas — pero viven en el modelo de oráculo, en problemas artificiales, no tareas por las que valga la pena pagar."}>
            {en
              ...
              : "BV necesita 1 consulta vs n clásicas; Deutsch–Jozsa 1 vs hasta 2ⁿ⁻¹+1; Simon O(n) vs ~2^{n/2}. A los tamaños que un navegador puede ejecutar, el tiempo clásico sigue siendo instantáneo — lo que enseñan honestamente es interferencia y phase kickback, no una aceleración práctica."}
          </Callout>
        ...
        <div className="method-body">
          ...
          <p>
            {en
              ...
              : "La factorización de Shor es QPE aplicada a la búsqueda de orden. Reduce factorizar N a hallar el orden multiplicativo r de una base a (el menor r con aʳ ≡ 1 mod N): QPE sobre el unitario de multiplicación modular U_a|y⟩ = |a·y mod N⟩ entrega una fase ≈ s/r, las fracciones continuas recuperan r, y los factores salen de gcd(a^{r/2}±1, N) cuando r es par y a^{r/2} ≢ −1. El build ejecuta la búsqueda de orden real para N=15 en las bases a ∈ {2,4,7,8,11,13}, construyendo U_a como un unitario de permutación 16×16 genuino y sus potencias controladas — recuperando r = 2 o 4 y factorizando 15 = 3×5 en las seis (la base 14 se excluye porque 14 ≡ −1 da solo el factor trivial)."}
          </p>
          <Callout
            title={en ? "Quantum vs classical —" : "Cuántico vs clásico —"}
            pt={en
        ...
        <div className="method-body">
          ...
          <p>
            {en
              ...
              : "Geométricamente, una iteración de Grover G = D·O es una rotación por 2θ en el plano 2-D generado por los estados uniformes marcado y no-marcado, donde sin θ = √(M/N). Tras k iteraciones la amplitud marcada es sin((2k+1)θ), así que la probabilidad de éxito sube, llega al máximo, y — crucialmente — vuelve a bajar si se sobre-rota. El punto óptimo de parada es k* = round((π/2 − θ)/(2θ)) ≈ (π/4)√(N/M). Las trazas versionadas reproducen los valores exactos de libro: N=4, M=1 es exacto en una iteración (P = 1.000); N=8 alcanza 0.945 en k=2; N=16 alcanza 0.961 en k=3; un caso N=8 de dos marcados llega a 1.000 en una sola iteración."}
          </p>
          <Eq
            tex={String.raw`k^*=\operatorname{round}\!\left(\frac{\pi/2-\theta}{2\theta}\right)\approx\frac{\pi}{4}\sqrt{\frac{N}{M}}`}
            caption={{
              en: "The optimal iteration count — run past k* and the marked amplitude falls back (the over-rotation visible live on the App).",
              es: "El conteo óptimo de iteraciones — pasado k* la amplitud marcada vuelve a caer (la sobre-rotación visible en vivo en la App).",
            }}
          />
          <p>
            {en
        ...
        <div className="method-body">
          ...
          <p>
            {en
              ...
              : "El optimizador clásico ajusta los ángulos (γ, β) para maximizar el costo medido F(γ,β) = ⟨γ,β|H_C|γ,β⟩, y el corte propuesto se lee del bitstring más probable. El build ejecuta p=1 con una búsqueda exacta en grilla sobre (γ,β) por statevector — un barrido determinista de 24×24 = 576 evaluaciones — así que el paisaje de energía en la App es la grilla real versionada, no una caricatura. Esto es lo que significa 'aprendido' aquí: no hay ángulos de compuerta fijos; los ángulos se entrenan contra un objetivo medido, y el paisaje (γ,β) es la superficie de optimización que el algoritmo escala."}
          </p>
          <Eq
    ...
    /* ─────────────────────────── 7 · Quantum ML (learned) ──────────────── */
    {
      ...
      content: (
        <div className="method-body">
          ...
          <p>
            {en
              ? "The Gram matrix feeds sklearn.SVC(kernel='precomputed'), and the honest comparison is against a classical sklearn.SVC(kernel='rbf') on the same raw features. The build runs six datasets of increasing difficulty — linear, linear-hard (overlapping), circles (concentric), moons, xor (4 clusters), blobs — and reports train/test accuracy for both classifiers over identical splits at seed 42. The two decision boundaries are drawn over the same points, and the accuracy decides."
              : "La matriz de Gram alimenta sklearn.SVC(kernel='precomputed'), y la comparación honesta es contra un clásico sklearn.SVC(kernel='rbf') sobre las mismas características crudas. El build ejecuta seis datasets de dificultad creciente — linear, linear-hard (solapado), circles (concéntricos), moons, xor (4 clústeres), blobs — y reporta exactitud train/test para ambos clasificadores sobre las mismas particiones con semilla 42. Las dos fronteras de decisión se trazan sobre los mismos puntos, y la exactitud decide."}
          </p>
          <Eq
        ...
        <div className="method-body">
          ...
          <Callout
            ...
            pt={en
              ...
              : "La QEC sí escala (Google Willow cruzó el umbral, 2024) — pero fue un solo qubit lógico; una máquina útil necesita cientos-a-miles, cada uno ~1000 qubits físicos, a ~8 órdenes de ser útil."}>
            {en
