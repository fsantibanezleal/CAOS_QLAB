...
**JavaScript state-vector simulator** (`quantum-circuit`, MIT) in a Web Worker. Moving a slider (a rotation
angle, a Grover iteration count) re-simulates the circuit and redraws the Bloch sphere / amplitude bars /
histogram instantly. The output is a `Trace` of the same shape the offline pipeline produces, so the
renderer is identical — *"live" is slider-responsiveness, not a different model.*
...
is JS; the real Python engines stay in the offline precompute lane.
